...
import { payrollApproveSchema } from "@/modules/payroll/validation/schemas";
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = payrollApproveSchema.safeParse(body);
    if (!parsed.success) return apiError("Validation failed", 422);
    const row = await getPayrollServices(auth.companyId).payroll.approve(parsed.data.payrollId, parsed.data.version, auth.userId);
