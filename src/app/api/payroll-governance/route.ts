...
export async function POST(request: Request) {
  try {
    ...
    if (action === "approval") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).approval.processAction(parsed.data, auth.id));
    }
    ...
    if (action === "lock") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).lock.lock(parsed.data, auth.id));
    }
    ...
    if (action === "unlock") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).lock.unlock(parsed.data, auth.id));
    }
    ...
    if (action === "reopen") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).lock.reopen(parsed.data, auth.id));
    }
    ...
    if (action === "snapshot") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).snapshot.create(parsed.data, auth.id));
    }
    ...
    if (action === "rollback") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).version.rollback(parsed.data, auth.id));
    }
    ...
    if (action === "retro") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).retro.create(parsed.data, auth.id));
    }
    ...
    if (action === "arrear") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).arrear.create(parsed.data, auth.id));
    }
    ...
    if (action === "financial_year") {
      ...
      if (body.subAction === "create") {
        ...
        if (!parsed.success) return apiError("Validation failed", 422);
        return apiSuccess(await getPayrollGovernanceServices(auth.companyId).financialYear.create(parsed.data, auth.id));
      }
      const parsed = financialYearActionSchema.safeParse(body);
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).financialYear.processAction(parsed.data, auth.id));
    }
    ...
    if (action === "compliance_snapshot") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).compliance.capture(parsed.data, auth.id));
    }
    ...
    if (action === "backup") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).backup.create(parsed.data, auth.id));
    }
    ...
    if (action === "restore_backup") {
      ...
      if (!parsed.success) return apiError("Validation failed", 422);
      return apiSuccess(await getPayrollGovernanceServices(auth.companyId).backup.restore(parsed.data, auth.id));
    }
